...
echo "$($PSQL "SELECT sum(total) FROM (SELECT winner_goals + opponent_goals AS total FROM games) AS totals")"
...
echo "$($PSQL "SELECT AVG(total) FROM (SELECT winner_goals + opponent_goals AS total FROM games) AS totals")"
